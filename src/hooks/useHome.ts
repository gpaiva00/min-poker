...
export function useHome({ start }: { start?: boolean }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToParticipantRemovals(userData.userId, roomId => {
      // Remover a sala da lista de salas participadas
      setParticipatedRoomIds(prev => {
        const updated = prev.filter(id => id !== roomId)
        return updated
  ...
  async function handleJoinRoom(name: string) {
    ...
    const success = await joinExistingRoom(pendingRoomId, userData.userId, name)
    if (success) {
  ...
  async function handleLeaveRoom() {
    ...
    try {
      ...
      setUserRooms(prev => prev.filter(room => room.id !== selectedRoom.id))

      // Remover a sala da lista de salas participadas
      setParticipatedRoomIds(prev => prev.filter(id => id !== selectedRoom.id))
    } catch (error) {
